...
homepage () { curl -s https://dragondevsmlh.duckdns.org; }
register () { curl -s -d "username=$1&password=$2" -X POST https://dragondevsmlh.duckdns.org/register; }
login () { curl -s -d "username=$1&password=$2" -X POST https://dragondevsmlh.duckdns.org/login; }

assert "$(homepage)" "MLH DRAGON BLOG"
